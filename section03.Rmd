...
d2 <- d %>% filter(year(date) == 2015)
m + 
  geom_point(data = d2,aes(long, lat, size = catch), col = "red", alpha = 0.3) +
  scale_size_area(max_size = 15)
p <- m + geom_point(data = d2, aes(long, lat, colour = length))
p + scale_color_gradientn(colours = rainbow(5))
p + scale_color_gradient(low = "red",high = "blue", trans = "sqrt")


p <- ggplot(d2)
p + geom_point(aes(depth,catch))
p + geom_point(aes(depth,temperature, colour = lat>65))
p + geom_point(aes(depth,temperature, colour = lat>65))
