...
\frametitle{Other items}
  ...
  \vs
  \item \textbf{Plots}: Graphical output from R. The user can export these the figures to file or clipboard
\ei
}
...
\frametitle{Project}
  ...
  \vs
  \item Projects save the state between sessions. This inludes:
...
\frametitle{Turning R off}
  ...
  In general you should \textbf{not} save your workspace
...
\frame{
\frametitle{On help}
A help page usually has the following categories:
...
\frame{
  ...
  \vs
  \item In ggplot2, plots are built by layering graphical components
  \vs
...
Alternatively one can use these more sane defaults
...
With global data and aes mapping
...
<<eval=FALSE>>=
minke$new.col <- minke$ID + 3
@

or use mutate

<<eval=FALSE>>=
minke <- mutate(minke,ID = ID + 3)
@
